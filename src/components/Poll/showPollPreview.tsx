        ...
        {options.map((option) => {
          ...
          return (
            <div key={id} data-key={id}
              className={`${styles.previewOption} ${activeOption === id && styles.active}`}>
              {title}
            </div>
          )
        })}
      </div>
      <Button label='Vote' onClick={handleSubmit} primary={activeOption !== ''} disabled={activeOption === ''}/>
    </div>
  )
}
...
function ShowPollPreview({pollData, handlePollSubmit}: IPreviewComponent) {
  ...
  return (
    <>
      {showPreview && <PreviewContent pollData={pollData} handlePollSubmit={handlePollSubmit} />}
